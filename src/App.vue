...
<template>
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        <div class="bg-[#1e293b] text-white rounded-2xl shadow-2xl border border-slate-700 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 p-4 sm:px-4 sm:py-3">
          <!-- Top row (Icon + label + Clear button) -->
          <div class="flex items-center gap-3 flex-1 min-w-0">
            ...
            <!-- Clear selection (shows on right side of top row on mobile) -->
            <button
              class="sm:hidden p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-slate-700 transition-colors flex-shrink-0 cursor-pointer"
          ...
          <!-- Bottom row of buttons on mobile, or same row on desktop -->
          <div class="flex items-center gap-2 sm:ml-auto">
            <!-- AI Study (only when compatible items are in cart) -->
            <button
              v-if="firstAiItem"
              class="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 sm:py-1.5 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 hover:opacity-90 text-white text-xs font-bold rounded-xl transition-all cursor-pointer shadow-md shadow-purple-500/10"
            ...
            <!-- Download All -->
            <button
              class="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 sm:py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold rounded-xl transition-colors disabled:opacity-60 cursor-pointer"
            ...
            <!-- Clear selection (shows on desktop) -->
            <button
              class="hidden sm:block p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-slate-700 transition-colors flex-shrink-0 cursor-pointer"
              title="Clear selection"
              @click="cart.clear()"
            >
              <Trash2 class="w-4 h-4" />
